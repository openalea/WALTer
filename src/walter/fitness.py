...
def flo_date(Transiflo_dico, nb_days = 30, nb_days_after = 0):
    ...
    for plante, val in list(Transiflo_dico.items()): # For each plant in the dico_stade dict
        ...
        for axis, val2 in list(val.items()): # For each axis in dico_stade dict
            ...
            Flo_date_dict[plante][axis] = list(range(date_flo-nb_days, date_flo + nb_days_after)) # For each axis of each plant, the dict is filled with a list of DOYs for which the PAR must be summed
...
def fitness(PAR_dict, Axes_dict, geno_dict, flo_date_dict, a_k=1, b_k=0):
    ...
    Final_PAR_fitness['Fitness_by_axis'] = old_div(Final_PAR_fitness['Abs_int_PAR'],Final_PAR_fitness['Temperature']) # (Fischer, 1985) The fitness (number of kernels) is proportional to the ratio mean(PAR)/mean(Temperature)
